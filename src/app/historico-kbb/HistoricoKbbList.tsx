...
// HistoricoKbbList.tsx — collapsible list of past KBB consultations.
//
// Each row is a clickable card showing placa + brand/model + consulted_at.
// Expanding reveals a quick summary (fair price + Molicar price + a deep
// link to /precos?placa=X which will cache-hit and render the full page).
...
export function HistoricoKbbList({ rows }: { rows: KbbConsultaRow[] }) {
  if (rows.length === 0) {
    return (
      <div className="surface p-8 text-center text-sm text-[var(--fg-muted)]">
        Sem consultas registradas. Use <Link href="/precos" className="underline">/precos</Link> para começar.
      </div>
    );
  }
  return (
    ...
    <li className="surface overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-[12rem]">
          ...
          {row.version && (
            <p className="text-xs text-[var(--fg-muted)] truncate max-w-[42rem]">{row.version}</p>
          )}
        </div>
        ...
        <span className={`text-[var(--fg-muted)] text-xs transition-transform ${open ? "rotate-90" : ""}`}>
          ▶
        </span>
      </button>
      ...
      {open && (
        <div className="border-t border-[var(--border)] p-4 grid gap-4 sm:grid-cols-[1fr_auto]">
          <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3 text-sm">
            <Cell label="Varejo (preço justo)" value={fair !== null ? BRL.format(fair) : "—"} mono />
            <Cell label="Preço Molicar" value={moli !== null ? BRL.format(moli) : "—"} mono />
          ...
          <div className="flex sm:flex-col gap-2 sm:items-end">
            <Link
              href={`/precos?placa=${encodeURIComponent(row.placa)}`}
